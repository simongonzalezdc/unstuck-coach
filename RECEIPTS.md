...
| It is state-aware without hidden judgment | `PROJECT_INSTRUCTIONS.md`, `rules.md`, `reference/mode-router.md`, `reference/admin-ops-playbooks.md`, `evals/red-face-tests.md`, `scripts/verify-mode-router.mjs`, `scripts/verify-eval-coverage.mjs` | Supplied calendar, inbox, task-pile, language, prior-conversation, and repeated-pattern facts can infer capacity, but the coach names uncertainty, lowers the next move, and avoids character judgments or diagnosis. |
