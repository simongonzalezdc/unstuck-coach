...
Use this when the submission needs a short video or GIF proof layer. `docs/walkthrough.md` is the longer tour; this file is the tight version a viewer can watch without losing the thread.
...
| 0:31-0:44 | Live demo with a stuck prompt | The hosted coach turns the prompt into one next move instead of giving an article. |
| 0:44-0:58 | Transcript pack, red-face evals, and ICM trace | The coach is tested under shame, overload, inbox/calendar reality, communication threat, and re-entry. |
| 0:58-1:15 | Evidence reader and scorecard | The evidence stays readable without slowing the first move. |
...
The first reply has to name the friction without blame, give one concrete move, hold the rest of the context, and ask for tiny proof. The live demo shows that loop. The transcript pack, red-face evals, first-run receipt, scorecard, product thesis, and ICM trace make the behavior inspectable.

The evidence reader renders the files behind the claim: first-run receipt, first-reply scorecard, transcripts, stress cases, safety boundaries, whole-person tour, and admin playbooks. The story ends on the coach and the evidence someone can inspect.
```

## Cut Rules

- Keep the reel under 75 seconds.
- Show the coach behavior before showing the evidence map.
- Show one stuck prompt, not a menu of features.
- Show `FIRST_REPLY_SCORECARD.md` before the evidence reader so pass and fail are clear.
