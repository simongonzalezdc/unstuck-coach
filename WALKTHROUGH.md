...
31. Run `node scripts/judge-quick-proof.mjs`.
32. Run `node scripts/final-review-smoke.mjs --expect-blocked`.
33. Point to the product-thesis evidence refs, rules-trace rows, pitch-reel rows, reel-page scenes, judge-FAQ questions, judge-scorecard rows, submission-surface sync, ICM-trace rows, first-run checks, first-reply scorecard checks, start-here prompt blocks, clean-stage preflight, judge quick proof summary, copy-button count, nine-case console behavior check, nine-case transcript check, and nine-case first-reply check in the verifier output.
34. Mention that `PUBLICATION_CHECKLIST.md` keeps the private-to-public posting lane explicit.
35. Mention that `node scripts/prepare-publication-link.mjs --url https://github.com/OWNER/REPO --write` inserts the approved public URL.
36. Mention that `node scripts/stage-public-repo.mjs --target ../startline-coach-week5-public` dry-runs the clean repository handoff.
37. Mention that `node scripts/verify-publication-ready.mjs` becomes the final green gate after the public GitHub link is inserted.
38. End on `docs/judge-walkthrough.md` and the four prompts a judge should try.
